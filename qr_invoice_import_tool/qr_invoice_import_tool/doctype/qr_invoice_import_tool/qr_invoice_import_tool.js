...
function handleQRScan(qrData, frm) {
    frappe.call({
        method: 'qr_invoice_import_tool.qr_invoice_import_tool.doctype.qr_invoice_import_tool.qr_invoice_import_tool.process_qr_data',
        args: {
            qr_data: qrData,
...
function fetchCompanyDefaults(company, frm) {
    frappe.db.get_value('Company', company, ['default_expense_account', 'default_income_account'], (values) => {
        if (values) {
            frm.set_value('default_expense_account', values.default_expense_account);
        }
    });
}
